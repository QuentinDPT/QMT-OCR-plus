...
public class CameraFactory : ICameraFactory
{
    ...
    {
        ArgumentNullException.ThrowIfNull(startupParameters);

        if (postAcquisitionParameters is null)
            postAcquisitionParameters = new();

        Guid cameraInstanceGuid = Guid.NewGuid();

        TCamera cameraInstance;

        cameraInstance = ActivatorUtilities.CreateInstance<TCamera>(_serviceProvider, postAcquisitionParameters, startupParameters);
